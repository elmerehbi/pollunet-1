...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 15:15:32 2017
...
gen_weight_dir = "/users/local/dnguyen/polluNet2/pix2pix/pix2pix_out/weights/gen_weights_epoch_90.h5"
"""
###########################
...
with h5py.File(hdf_data,"r") as f:
    ...
    for i in range(nb_shuffle):
        ...
        l = list(np.array(ws_idx_train)[l])
        ...
        for i_d in xrange(d):
            im_tmp = input_gmf[i,:,:].reshape(-1,1,im_height,im_width)
...
with h5py.File(hdf_data,"r") as f:    
    test_nrcs=f["test/Nrcs/testing_images/"][ws_idx_test2] # HDF5 dataset, shape = (5065, 508, 508)
    input_gmf = shape_modify(f["test/GMF/testing_images"][ws_idx_test2]) # HDF5 dataset, shape = (5065, 508, 508)
    input_ship = f["masks/testing_images/"+fl0][ws_idx_test2][...,1]  # np.array, shape = (5065, 508, 508)
    # GEN
    d,_,_ = input_gmf.shape
    input_gen = np.zeros((d,im_height,im_width))
    for i_d in xrange(d):
...
with h5py.File(hdf_data,"r") as f:
    test_nrcs=f["test/Nrcs/training_images/"][ws_idx_test1] 
    input_gmf = shape_modify(f["test/GMF/training_images"][ws_idx_test1] )
    input_ship = f["masks/training_images/"+fl0][ws_idx_test1][...,1]#.reshape(-1,size_out,size_out,2)
    # GEN
    d,_,_ = input_gmf.shape
    input_gen = np.zeros((d,im_height,im_width))
    for i_d in xrange(d):
