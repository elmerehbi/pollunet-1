...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 22 09:41:41 2017
...
with h5py.File(hdf_data,"r") as f:
    ...
    for i in range(nb_shuffle):
        ...
        for i_im in xrange(train_nrcs.shape[0]):
            train_nrcs[i_im,:,:] = ndimage.median_filter(train_nrcs[i_im,:,:], size = 7)
            # MEAN SHIFT
            input_gmf[i_im,:,:] += train_nrcs[i_im,:,:].mean() - input_gmf[i_im,:,:].mean() 
...
with h5py.File(hdf_data,"r") as f:    
    ...
    for i_im in xrange(test_nrcs.shape[0]):
        ...
        input_gmf[i_im,:,:] += test_nrcs[i_im,:,:].mean() - input_gmf[i_im,:,:].mean() 
