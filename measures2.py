...
def load_pollutions():
    ...
    with h.File(hdf,'a') as f:
        if f.__contains__('pollutions_indices2/'+tra):
            del f['pollutions_indices2/'+tra]
        if f.__contains__('pollutions_indices2/'+tr):
            del f['pollutions_indices2/'+tr]
        if f.__contains__('pollutions_indices2/'+ts):
            del f['pollutions_indices2/'+ts]
        f['pollutions_indices2/'+tra]=np.array(find_mask(6,tra,ws_idx_train))
        f['pollutions_indices2/'+tr]=np.array(find_mask(6,tr,ws_idx_test1))
        f['pollutions_indices2/'+ts]=np.array(find_mask(6,ts,ws_idx_test2))
...
with h.File(hdf,'r') as f:
    if f.__contains__('pollutions_indices2/'+ts):
        l4_train=list(f['pollutions_indices2/'+tra][:])
        l4_tr=list(f['pollutions_indices2/'+tr][:])
        l4_ts=list(f['pollutions_indices2/'+ts][:])
