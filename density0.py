...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 10 12:19:08 2017
...
density_input = Input(shape=(height,width))
...
density = Reshape((1,height,width))(density_input)

c = crop() # call this function before any unet_layers() use.
x = concatenate([sar,gmf,ship,density],axis=1)
...
unet = Model([sar_input,gmf_input,ship_input,density_input],x)
...
with h5py.File(hdf_data,"r") as f:
    ...
    for i in range(nb_shuffle):
        ...
        # Input gmf
        input_density = f["train/TrafficDensity"][l]        
        ...
        unet.fit([np.sqrt(train_nrcs),np.sqrt(input_gmf), input_ship, input_density],
                  train_mask,
                  shuffle=True,
                  verbose=1,
...
with h5py.File(hdf_data,"r") as f:    
    ...
    input_density = f["test/TrafficDensity/testing_images"]
    v=unet.predict([np.sqrt(test_nrcs),np.sqrt(input_gmf),input_ship,input_density],
                    verbose=1,
                    batch_size=16) #np.array, shape = (5044, 99856, 3) 99856 = 316**2
...
with h5py.File(hdf_data,"r") as f:
    ...
    input_density = f["test/TrafficDensity/training_images"]
    w=unet.predict([np.sqrt(test_nrcs),np.sqrt(input_gmf),input_ship,input_density],
                    verbose=1,
                    batch_size=16)
