...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 26 22:10:39 2017
...
###########################
"""
print datetime.datetime.utcnow(), 'begin leaning...'
...
with h5py.File(hdf,"r") as f:
    ...
    for i in range(nb_shuffle):
        ...
        patchwise_label = np.max(np.argmax(train_mask,axis = -1), axis = 1)
        # Re_create the list of training patches, in this list, the number of 
        # images without pollution and the images with pollution/ships are similar
        l2 = np.argwhere(patchwise_label != 0)
        l0 = np.argwhere(patchwise_label == 0)
        len_max = min(len(l2),len(l0))
        l2 = np.squeeze(np.concatenate((l2,l0[:len_max])))
        pm = np.random.permutation(len(l2))
        l2 = l2[pm]
        train_mask = train_mask[l2,...]
        # patchwise_label = (array([0, 1, 2]), array([1382,  386,  137]))]
        # Weights
        print "weights"
        w=f["weights/"+fl][l][l2] # np.array, shape = (1898, 99856)
        # Train nrcs
        print "nrcs"
        train_nrcs=f["train/Nrcs"][l][l2] # np.array, shape = (1898, 508, 508)
        # Input gmf
        print "gmf"
        input_gmf = f["train/GMF"][l][l2]
        print "weights"
        w[w==1.]=weight_boats
        w[w==0.]=weight_sea   
...
with h5py.File(hdf,"r") as f:    
    ...
    v=unet.predict([test_nrcs,input_gmf],verbose=1,batch_size=16) #np.array, shape = (5044, 99856, 3) 99856 = 316**2
...
with h5py.File(hdf,"r") as f:
    ...
    w=unet.predict([test_nrcs,input_gmf],verbose=1,batch_size=16)
