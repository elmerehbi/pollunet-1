...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  7 22:23:39 2017
...
print 'dropout_down', dropout_down
print 'dropout_up', dropout_up
...
with h5py.File(hdf_data,"r") as f:
    ...
    for i in range(nb_shuffle):
        ...
        unet.fit([10**(-3*train_nrcs),10**(-3*input_gmf), input_ship],train_mask,shuffle=True,verbose=1,batch_size=batch_size,epochs=epochs,sample_weight=w)
...
with h5py.File(hdf_data,"r") as f:    
    ...
    input_ship = f["masks/testing_images/"+fl0][:][...,1]  # np.array, shape = (5065, 508, 508)
     #np.array, shape = (5044, 99856, 3) 99856 = 316**2
    v=unet.predict([10**(-3*test_nrcs),10**(-3*input_gmf),input_ship],verbose=1,batch_size=16)
...
with h5py.File(hdf_data,"r") as f:
    ...
    w=unet.predict([10**(-3*test_nrcs),10**(-3*input_gmf),input_ship],verbose=1,batch_size=16)
